## Figure expression 
setwd("~/Documents/03_Syntenic_linkage/01_Data/synteny_network_results/figure_S6_coexpression_five_species_cluster_synteny/")
library(openxlsx)
...
ap_dis <- apcluster::apcluster(s = cor_tkm_rm, convits = 1000, 
    maxits = 100000, nonoise = FALSE, seed = 1000)
...
## cluster 1
cluster_level <- c("1", "2", "3", "4", "5", "7", "10", "11", "14", 
    "21", "22", "23", "27")

tkm_clust1 <- tkm[tkm[, "gene"] %in% names(ap_dis@clusters[[1]]), ][, 1:3]
## add 0 for those clusters that are not in the set
tkm_clust1 <- rbind(tkm_clust1, c(NA, "2", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "5", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "7", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "10", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "14", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "21", "0"))
tkm_clust1 <- rbind(tkm_clust1, c(NA, "22", "0"))
...
## add 0 for those clusters that are not in the set
tkm_clust2 <- rbind(tkm_clust2, c(NA, "1", "0"))
tkm_clust2 <- rbind(tkm_clust2, c(NA, "2", "0"))
tkm_clust2 <- rbind(tkm_clust2, c(NA, "3", "0"))
tkm_clust2 <- rbind(tkm_clust2, c(NA, "7", "0"))
tkm_clust2 <- rbind(tkm_clust2, c(NA, "11", "0"))
tkm_clust2 <- rbind(tkm_clust2, c(NA, "27", "0"))
...
tkm_clust3 <- rbind(tkm_clust3, c(NA, "10", "0"))
tkm_clust3 <- rbind(tkm_clust3, c(NA, "14", "0"))
tkm_clust3 <- rbind(tkm_clust3, c(NA, "21", "0"))
tkm_clust3 <- rbind(tkm_clust3, c(NA, "22", "0"))
tkm_clust3 <- rbind(tkm_clust3, c(NA, "23", "0"))
...
## add 0 for those clusters that are not in the set
tkm_clust4 <- rbind(tkm_clust4, c(NA, "1", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "2", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "3", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "11", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "14", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "21", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "23", "0"))
tkm_clust4 <- rbind(tkm_clust4, c(NA, "27", "0"))
