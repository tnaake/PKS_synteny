...
genes_table <- read.table("./Results_Nov05/family_orthogroups_iadhore.txt", sep = "\t", 
    header = FALSE, stringsAsFactors = FALSE)
## for OrthoFinder
pks_genes <- sort(genes_table[genes_table[, 2] == "OG0000256", 1]) 
genes_table_mcl <- read.table("./Results_Nov05/family_orthogroups_mcl_iadhore.txt", 
...
setwd("~/winhome/Documents/03_Syntenic_linkage/01_Data/OrthoFinder/input/fasta_all_GO_output")
go_files <- list.files()
...
attr <- attr[attr[, "connecting_largenetwork"] == "yes", ]
attr <- attr[attr[, "classification_community_apclust_dist"] != "-", ]

## test for enrichment --> take clusters that have at least 15% R-4-C and more than 10 R-4-C --> 2, 6, 14, 29
clust_total <- table(unlist(strsplit(
    attr[!attr[, "classification_community_apclust_dist"] %in% "-", "type_pPAP_reviewed"], "/")))
clust_24514 <- table(unlist(strsplit(
    attr[attr[, "classification_community_apclust_dist"] %in% c("2", "4", "5", "14"), "type_pPAP_reviewed"], "/")))
freq_chs <- matrix(c(clust_24514["R-4-C"], sum(clust_24514) - clust_24514["R-4-C"], 
    clust_total["R-4-C"] - clust_24514["R-4-C"], 
    sum(clust_total) - sum(clust_24514) - (clust_total["R-4-C"] - clust_24514["R-4-C"])), 
    byrow = TRUE, ncol = 2)
colnames(freq_chs) <- c("R-4-C", "!R-4-C")
rownames(freq_chs) <- c("24514", "non24514")
...
## 1. get all genes in genes_fg, clusters that contain high amount of CHS are 2, 4, 5, 14
genes_fg2 <- attr[attr[, "classification_community_apclust_dist"] %in% c(2, 4, 5, 14), "gene_tandem"]
...
g <- ggplot(df_freq) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0), 
        axis.text.y = element_text(angle = 90)) + ylim(0, 6.3)
ggsave(plot = g, device = "pdf", 
...
g <- ggplot(df_all_r) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0), 
         axis.text.y = element_text(angle = 90))
ggsave(plot = g, filename = "figure_S3_GO_PKS_removed/barplot_all_C_removedPKS.pdf", device = "pdf")
...
g <- ggplot(df_freq_r) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0, size = 5), 
        axis.text.y = element_text(angle = 90)) + ylim(0, 8.7)
ggsave(plot = g, device = "pdf", 
    filename = "figure_S3_GO_PKS_removed/barplot_frequency_C_freq_removedPKS.pdf")
...
g <- ggplot(df_BP_r) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
        axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_S3_GO_PKS_removed/barplot_BP_C_removedPKS.pdf", device = "pdf")
## MF
g <- ggplot(df_MF_r) + 
    geom_bar(aes(x = term, y = pvalue), stat = "identity", width = 0.85) + 
    ylim(0, 40) +
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0, size = 5), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_S3_GO_PKS_removed/barplot_MF_C_removedPKS.pdf", device = "pdf")
## CC
g <- ggplot(df_CC_r) + 
    geom_bar(aes(x = term, y = pvalue), stat = "identity", width = 0.85) + 
    ylim(0, 40) +
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
        axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_S3_GO_PKS_removed/barplot_CC_C_removedPKS.pdf", device = "pdf")
...
g <- ggplot(df_freq_r) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0, size = 5), 
        axis.text.y = element_text(angle = 90)) + ylim(0, 8.7)
ggsave(plot = g, device = "pdf", 
    filename = "figure_S3_GO_PKS_removed/barplot_frequency_C_freq_smaller_removedPKS.pdf")

## write objects to files 
## for A
write.table(go_A_r, file = "figure_S3_GO_PKS_removed/go_A_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_A_MF_r, file = "figure_S3_GO_PKS_removed/go_A_MF_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_A_CC_r, file = "figure_S3_GO_PKS_removed/go_A_CC_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_A_BP_r, file = "figure_S3_GO_PKS_removed/go_A_BP_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)

## for B
write.table(go_B_r, file = "figure_S3_GO_PKS_removed/go_B_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_B_MF_r, file = "figure_S3_GO_PKS_removed/go_B_MF_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_B_CC_r, file = "figure_S3_GO_PKS_removed/go_B_CC_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_B_BP_r, file = "figure_S3_GO_PKS_removed/go_B_BP_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)

## for C
write.table(go_C_r, file = "figure_S3_GO_PKS_removed/go_C_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_C_MF_r, file = "figure_S3_GO_PKS_removed/go_C_MF_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_C_CC_r, file = "figure_S3_GO_PKS_removed/go_C_CC_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_C_BP_r, file = "figure_S3_GO_PKS_removed/go_C_BP_PKSremoved.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
...
## O. sativa
os05g12180 <- unlist(lapply(strsplit(os05g12180, split = "orysa_"), "[", 2))
os11g32650 <- unlist(lapply(strsplit(os11g32650, split = "orysa_"), "[", 2))
...
write.table(fasta_os12g07690, quote = FALSE, row.names = FALSE, col.names = FALSE,
    file = "figure_S5_co_expression_STRING/other_clusters/list_os12g07690.txt")

################################################################################
###################### using all proteins as background ########################
################################################################################
############## Gene enrichment (using all proteins as background) ##############
################################################################################
...
## get pks genes from output of OrthoFinder or MCL
genes_table <- read.table("./Results_Nov05/family_orthogroups_iadhore.txt", 
    sep = "\t", header = FALSE, stringsAsFactors = FALSE)
...
## remove orthogroups with membersize == 1, assign the cut gene tables to 
## genes_table_cut and genes_table_mcl_cut
of_filter <- names(table(genes_table[, 2])[table(genes_table[, 2]) == 1])
mcl_filter <- names(table(genes_table_mcl[, 2])[table(genes_table_mcl[, 2]) == 1])

genes_table_cut <- genes_table[!genes_table[, 2] %in% of_filter, ]
genes_table_mcl_cut <- genes_table_mcl[!genes_table_mcl[, 2] %in% mcl_filter, ]

## concatenate the two cut gene tables 
genes_bg_of <- unique(genes_table_cut[, 1])
genes_bg_mcl <- unique(genes_table_mcl_cut[, 1])

## i-ADHore
find_syntenic_regions_iadh_bg_all <- function(files, pks_genes, genes_bg) {
    
    genes_bg_final <- character()
    ## create vector to store syntenic genes (background)
    res <- vector("list", length(files))
    names(res) <- unlist(lapply(strsplit(files, split = "output_"), "[", 2))
    ...
    for (i in seq_along(files)) {
        ...
        ## cut species out of files and grep for the corresponding genes in 
        ## genes_bg
        species_1 <- strsplit(files[i], split = "_")
        species_1 <- unlist(lapply(species_1, "[", 2))
        species_2 <- strsplit(files[i], split = "_")
        species_2 <- unlist(lapply(species_2, "[", 3))
        
        genes_species_1 <- genes_bg[grep(x = genes_bg, pattern = species_1)]
        genes_species_2 <- genes_bg[grep(x = genes_bg, pattern = species_2)]
        
        ## concatenate to bg_species
        bg_species <- c(genes_species_1, genes_species_2)
	
        res[[i]] <- list(character())##, character())
        names(res[[i]]) <- c("fg") ## stores foreground
        ...
        genes_bg_final <- unique(c(genes_bg_final, bg_species))
    }
    ## return list with proteins per pks gene (length == length(pks_genes_all))
    ## return vector with all syntenic genes 
    return(list(res, genes_bg_final))
}

## MCScanX
## functions 
find_syntenic_regions_mcsc_bg_all <- function(files, pks_genes, genes_bg) {
    ...
    for (i in seq_along(files)) {
        ...
        ## genes_bg
        species_1 <- strsplit(files[i], split = "_")
        species_1 <- unlist(lapply(species_1, "[", 1))
        species_1 <- unlist(strsplit(species_1, split = "[.]collinearity"))
        species_2 <- strsplit(files[i], split = "_")
        species_2 <- unlist(lapply(species_2, "[", 2))
        species_2 <- unlist(strsplit(species_2, split = "[.]collinearity"))
...
output_files <- list.files()[grep(list.files(), pattern = "output_")]
## use here pks_genes 
reg_iadh_ofi <- find_syntenic_regions_iadh_bg_all(files = output_files, 
    pks_genes = pks_genes, genes_bg = genes_bg_of) 

## for MCL input
setwd("../output_mcl")
output_files <- list.files()[grep(list.files(), pattern = "output_")]
## use here pks_genes_mcl
reg_iadh_mcl <- find_syntenic_regions_iadh_bg_all(files = output_files, 
    pks_genes = pks_genes_mcl, genes_bg = genes_bg_mcl) 

## for orthoFinder input
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_MCScanX/MCScanX_gff")
collinearity_files <- list.files()[grep(list.files(), pattern = "[.]collinearity")]
## use here pks_genes 
reg_mcsc_ofi <- find_syntenic_regions_mcsc_bg_all(files = collinearity_files, 
    pks_genes = pks_genes, genes_bg = genes_bg_of)

## for MCL input 
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_MCScanX/MCScanX_gff_mcl")
collinearity_files <- list.files()[grep(list.files(), pattern = "[.]collinearity")]
reg_mcsc_mcl  <- find_syntenic_regions_mcsc_bg_all(files = collinearity_files, 
    pks_genes = pks_genes_mcl, genes_bg = genes_bg_mcl)

## make the union from all results --> one object that stores the syntenic 
## genes per species-pair and gene
setwd("~/winhome/Documents/03_Syntenic_linkage/01_Data/synteny_network_results/figure_GO_enrichment_terms_bg_all")
save(reg_iadh_ofi, reg_iadh_mcl, reg_mcsc_ofi, reg_mcsc_mcl, 
     file = "find_syntenic_regions_iadh_mcsc_bg_all.RData")

## write background to objects and 
bg_iadh_ofi <- reg_iadh_ofi[[2]]
reg_iadh_ofi <- reg_iadh_ofi[[1]]

bg_iadh_mcl <- reg_iadh_mcl[[2]]
reg_iadh_mcl <- reg_iadh_mcl[[1]]

bg_mcsc_ofi <- reg_mcsc_ofi[[2]]
reg_mcsc_ofi <- reg_mcsc_ofi[[1]]

bg_mcsc_mcl <- reg_mcsc_mcl[[2]]
reg_mcsc_mcl <- reg_mcsc_mcl[[1]]

## remove tandem duplicates from bg 

## define functions
remove_tandem_bg_iadh <- function(bg) {
    
    ## get all files in the directory
    files <- list.files()
    files <- files[grep(files, pattern = "output")]
    
    for (i in 1:length(files)) {
        
        ## read file
        file_read <- paste0(files[i], "/genes.txt")
        genes <- tryCatch(read.csv(file_read, sep = "\t", 
            stringsAsFactors = FALSE, row.names = NULL, fill = TRUE), 
            error = function(x) NULL)
        
        if (is.null(genes)) {
            
            ## only show those that are tandems
            tandem <- genes[genes[, "is_tandem"] == -1, ]
            
            ## select those genes that are not tandem representatives
            tandem_remove <- tandem[tandem[, "is_tandem_representative"] == 0, ]
            tandem_remove <- tandem_remove[, "id"]
            
            ## remove those in bg that match to tandem_remove
            bg <- bg[!bg %in% tandem_remove]
        }
    }
    return(bg)
}

remove_tandem_bg_mcs <- function(bg) {
    ## get all files in the directory
    files <- list.files()
    files <- files[grep(files, pattern = "collinearity")]
    files <- unlist(strsplit(files, split = "[.]collinearity"))
    
    for (i in 1:length(files)) {
        
        ## read file
        file_read <- paste0(files[i], ".tandem")
        tandem <- tryCatch(read.csv(file_read, sep = ",", 
            stringsAsFactors = FALSE, row.names = NULL, fill = TRUE), 
            error = function(x) NULL)
        ...
        if (is.null(tandem)) {
            ...
            ## 
            ## how to remove
            ## find duplicated in vector (a2, b2, b3, d2)
            tandem_vector <- c(tandem[, 1], tandem[, 2])
            dupl <- duplicated(tandem_vector)
            
            ## remove the first instance of the duplicated
            dup <- tandem_vector[dupl]
            
            ## remove the line where the duplicated dup is (dup is from the 
            ## second col, remove the row where dup is in the first col)
            inds <- which(tandem[, 1] %in% dup)
            
            ## get all ids that should be removed
            tandem_remove <- c(dup, tandem[inds, 1], tandem[inds, 2])
            
            ## remove dup in bg that match to tandem_remove
            bg <- bg[!bg %in% tandem_remove]
        }
    }
    return(bg)
...
## iadhore/ofi
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_iadhore/output/")
bg_iadh_ofi <- remove_tandem_bg_iadh(bg_iadh_ofi)
## iadhore/mcl
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_iadhore/output_mcl/")
bg_iadh_mcl <- remove_tandem_bg_iadh(bg_iadh_mcl)

## mcscanx/ofi
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_MCScanX/MCScanX_gff")
bg_mcsc_ofi <- remove_tandem_bg_mcs(bg_mcsc_ofi)
## mcscanx/mcl
setwd("~/AG-Fernie/Thomas/Data/synteny/synteny_MCScanX/MCScanX_gff_mcl")
bg_mcsc_mcl <- remove_tandem_bg_mcs(bg_mcsc_mcl)

setwd("~/winhome/Documents/03_Syntenic_linkage/01_Data/synteny_network_results/figure_GO_enrichment_terms_bg_all")
save(bg_iadh_ofi, bg_iadh_mcl, bg_mcsc_ofi, bg_mcsc_mcl, file = "bg_removed_duplicates.RData")
...
bg_all <- unique(c(bg_iadh_ofi, bg_iadh_mcl, bg_mcsc_ofi, bg_mcsc_mcl))
## lapply(1:length(reg_iadh_ofi), function(x) {unique(
#     c(reg_iadh_ofi[[x]][[2]], reg_iadh_mcl[[x]][[2]], 
#       reg_mcsc_ofi[[x]][[2]], reg_mcsc_mcl[[x]][[2]])) 
# })
names(bg_all) <- names(reg_iadh_ofi)
...
## GO enrichment
## load list of: GO terms to genes
setwd("~/winhome/Documents/03_Syntenic_linkage/01_Data/OrthoFinder/input/fasta_all_GO_output")
load("go_terms_species_genes.RData")
...
## A) foreground #1 vs background (PKS containing regions=union of the four 
## methods vs background=all genes in proteome after removal of duplicates)
## 1. get all genes in genes_fg
genes_fg1 <- attr[, "gene_tandem"]
genes_fg1 <- unlist(strsplit(genes_fg1, split = "/"))
...
fg_all_A <- fg_all[inds_A]
bg_all_A <- bg_all[grep(bg_all, pattern = paste(species_fg1, collapse = "|"))]
...
fg_all_B <- fg_all[inds_B]
bg_all_B <- bg_all[grep(bg_all, pattern = paste(species_fg2, collapse = "|"))]
...
## for background
bg_all_B_uniq <- unique(unlist(bg_all_B))
bg_all_B_uniq_go <- lapply(1:length(bg_all_B_uniq), function(x) { 
    species <- lapply(strsplit(bg_all_B_uniq[x], split = "_"), "[", 1)
    species <- paste(unlist(species), "_GO.out", sep = "")
    unlist(go_l[[species]][ bg_all_B_uniq[[x]] ])
})

## 2.7 do enrichment test (bg is bg_all_B_uniq_go = bg for species present in 
## syntenic clusters 2, 3 and 5)
B_fisher <- fisher_test_all(bg = bg_all_B_uniq_go, fg = fg_all_B_uniq_go)
B_fisher_p <- unlist(lapply(B_fisher, function(x) x$p_values_adj))
B_fisher_p[B_fisher_p < 0.05]
...
## dim(df_CC) 31 3
## plot top 20 terms
setwd("/home/mpimp-golm.mpg.de/naake/winhome/Documents/03_Syntenic_linkage/01_Data/synteny_network_results")
df_all <- rbind(df_BP[order(df_BP[, 2], decreasing = TRUE)[1:20], ], 
                df_MF[order(df_MF[, 2], decreasing = TRUE)[1:20], ], 
                df_CC[order(df_CC[, 2], decreasing = TRUE)[1:20], ])
g <- ggplot(df_all) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, device = "pdf", 
    filename = "figure_GO_enrichment_terms_bg_all/barplot_all_C_bg_all.pdf")
...
g <- ggplot(df_BP) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, device = "pdf", 
    filename = "figure_GO_enrichment_terms_bg_all/barplot_BP_C_bg_all.pdf")
...
g <- ggplot(df_MF) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
          axis.text.y = element_text(angle = 90))
ggsave(plot = g, device = "pdf", 
    filename = "figure_GO_enrichment_terms_bg_all/barplot_MF_C_bg_all.pdf")
...
g <- ggplot(df_CC) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
          axis.text.y = element_text(angle = 90))
ggsave(plot = g, device = "pdf", 
    filename = "figure_GO_enrichment_terms_bg_all/barplot_CC_C_bg_all.pdf")
...
g <- ggplot(df_freq) + 
    geom_bar(aes(x = name, y = freq, fill = is), stat = "identity", 
             width = 0.85, position = position_dodge(width = 1)) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0), 
          axis.text.y = element_text(angle = 90)) + ylim(0, 15.35)
ggsave(plot = g, device = "pdf", 
    filename = "figure_GO_enrichment_terms_bg_all/barplot_frequency_C_freq_bg_all.pdf")

## write objects to files 
write.table(gmt, file = "figure_GO_enrichment_terms_bg_all/go_gmt_bg_all.gmt", sep = "\t", dec = ".", 
            row.names = FALSE, quote = FALSE)

## terms for A
write.table(go_A, file = "figure_GO_enrichment_terms_bg_all/go_A_bg_all.txt", 
    sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_A_MF, file = "figure_GO_enrichment_terms_bg_all/go_A_MF_bg_all.txt", 
    sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_A_CC, file = "figure_GO_enrichment_terms_bg_all/go_A_CC_bg_all.txt", 
    sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_A_BP, file = "figure_GO_enrichment_terms_bg_all/go_A_BP_bg_all.txt", 
    sep = "\t", dec = ".", row.names = FALSE, quote = FALSE) 

## terms for B
write.table(go_B, file = "figure_GO_enrichment_terms_bg_all/go_B_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_MF, file = "figure_GO_enrichment_terms_bg_all/go_B_MF_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_CC, file = "figure_GO_enrichment_terms_bg_all/go_B_CC_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_BP, file = "figure_GO_enrichment_terms_bg_all/go_B_BP_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE) 

## terms for C
write.table(go_C, file = "figure_GO_enrichment_terms_bg_all/go_C_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_C_MF, file = "figure_GO_enrichment_terms_bg_all/go_C_MF_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_C_CC, file = "figure_GO_enrichment_terms_bg_all/go_C_CC_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_C_BP, file = "figure_GO_enrichment_terms_bg_all/go_C_BP_bg_all.txt", sep = "\t", dec = ".", row.names = FALSE, quote = FALSE)
...
## background 
bg_all_B_uniq_r <- bg_all_B_uniq[!bg_all_B_uniq %in% pks_genes_all]
bg_all_B_uniq_go_r <- lapply(1:length(bg_all_B_uniq_r), function(x) { 
    species <- lapply(strsplit(bg_all_B_uniq_r[x], split = "_"), "[", 1)
    species <- paste(unlist(species), "_GO.out", sep = "")
    unlist(go_l[[species]][ bg_all_B_uniq_r[[x]] ])
})

## 2.7 do ennrichment test
B_fisher_r <- fisher_test_all(bg = bg_all_B_uniq_go_r, fg = fg_all_B_uniq_go_r)
B_fisher_p_r <- unlist(lapply(B_fisher_r, function(x) x$p_values_adj))
B_fisher_p_r[B_fisher_p_r < 0.05]
...
g <- ggplot(df_all_r) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0), 
          axis.text.y = element_text(angle = 90))
ggsave(plot = g, filename = "figure_GO_enrichment_terms_bg_all/barplot_all_C_removedPKS_bg_all.pdf", device = "pdf")
...
g <- ggplot(df_freq_r) + 
    geom_bar(aes(x = name, y = freq, fill = is), stat = "identity", 
             width = 0.85, position = position_dodge(width = 1)) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0, size = 5), 
          axis.text.y = element_text(angle = 90)) + ylim(0, 15.46)
ggsave(plot = g, device = "pdf", 
       filename = "figure_GO_enrichment_terms_bg_all/barplot_frequency_C_freq_removedPKS_bg_all.pdf")
...
g <- ggplot(df_BP_r) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_GO_enrichment_terms_bg_all/barplot_BP_C_removedPKS_bg_all.pdf", device = "pdf")
## MF
g <- ggplot(df_MF_r) + 
    geom_bar(aes(x = term, y = pvalue), stat = "identity", width = 0.85) + 
    ylim(0, 30) +
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0, size = 5), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_GO_enrichment_terms_bg_all/barplot_MF_C_removedPKS_bg_all.pdf", device = "pdf")
## CC
g <- ggplot(df_CC_r) + 
    geom_bar(aes(x = term, y = pvalue), stat = "identity", width = 0.85) + 
    ylim(0, 30) +
    theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0), 
          axis.text.y = element_text(angle = 90))  
ggsave(plot = g, filename = "figure_GO_enrichment_terms_bg_all/barplot_CC_C_removedPKS_bg_all.pdf", device = "pdf")
...
g <- ggplot(df_freq_r) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0, size = 5), 
          axis.text.y = element_text(angle = 90)) + ylim(0, 15.46)
ggsave(plot = g, device = "pdf", 
       filename = "figure_GO_enrichment_terms_bg_all/barplot_frequency_C_freq_smaller_removedPKS_bg_all.pdf")

## write objects to files 
## for A
write.table(go_A_r, file = "figure_GO_enrichment_terms_bg_all/go_A_PKSremoved_bg_all.txt", sep = "\t", dec = ".", 
    row.names = FALSE, quote = FALSE)
write.table(go_A_MF_r, file = "figure_GO_enrichment_terms_bg_all/go_A_MF_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_A_CC_r, file = "figure_GO_enrichment_terms_bg_all/go_A_CC_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_A_BP_r, file = "figure_GO_enrichment_terms_bg_all/go_A_BP_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)

## for B
write.table(go_B_r, file = "figure_GO_enrichment_terms_bg_all/go_B_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_MF_r, file = "figure_GO_enrichment_terms_bg_all/go_B_MF_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_CC_r, file = "figure_GO_enrichment_terms_bg_all/go_B_CC_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_B_BP_r, file = "figure_GO_enrichment_terms_bg_all/go_B_BP_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)

## for C
write.table(go_C_r, file = "figure_GO_enrichment_terms_bg_all/go_C_PKSremoved_bg_all.txt", sep = "\t", dec = ".", 
            row.names = FALSE, quote = FALSE)
write.table(go_C_MF_r, file = "figure_GO_enrichment_terms_bg_all/go_C_MF_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_C_CC_r, file = "figure_GO_enrichment_terms_bg_all/go_C_CC_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
write.table(go_C_BP_r, file = "figure_GO_enrichment_terms_bg_all/go_C_BP_PKSremoved_bg_all.txt", sep = "\t", 
    dec = ".", row.names = FALSE, quote = FALSE)
