...
tree <- read.newick("HMMer_pHMM/booster_new.nw", node.label = "support")	
tree@data$support[1:2] <- 1

## load supplementary information
pks_genes <- openxlsx::read.xlsx("./HMMer_pHMM/pks_genes_tree_id_type.xlsx", 
    sheet = 1)
...
## find indices in attr that match to tipLabel (gives row indices in attr where 
## the entries are for tipLabel)
...
cluster235 <- lapply(cluster, function(x) {
    ifelse(x == "NA", x, ifelse(x %in% c("2", "3", "5"), x, "not"))})
cluster <- unlist(cluster)
cluster235 <- unlist(cluster235)
...
inds <- lapply(tipLabel, function(x) {
    grep(pks_genes_ID, pattern = paste(strsplit(x, split="/")[[1]], collapse = "|"))}) ##which(pks_genes_ID %in% unlist(strsplit(x, split="/"))))
names(inds) <- tipLabel

## get pPAP_reviewed
pPAP <- unlist(lapply(inds, function(x) {
    paste(unique(pks_genes[x, "pPAP_reviewed"]), collapse = "/")}))
...
p <- ggtree:::layout_circular(p)
p <- p + 
    geom_tiplab(label = tree@phylo$tip.label, aes(angle = angle), size = 0.2) + 
    #theme(legend.position = "right") + 
    scale_color_continuous(low = "red", high = "green") 

## add additional information that was created previously 
info <- data.frame(cluster = cluster, cluster235 = cluster235, is = pPAP, 
    exon = numberexon, class = class, unranked = unranked, order = Order, 
    family = family)
rownames(info) <- tree@phylo$tip.label


## function to create a plot with a legend (taken from ggtree and modified)
gheatmap_mod <- function (p, data, offset = 0, width = 1, low = "green", 
    ...
    if (colnames) {
        ...
        p2 <- p2 + geom_text(data = mapping, aes(x = to, y = y, label = from), 
            size = font.size, inherit.aes = FALSE, angle = colnames_angle, 
...
## apply the function 
## plot with no legend
p2 <- gheatmap_mod(p, info, offset = 0.1, width = 0.1, font.size = 1, 
                   colnames_angle = -45, hjust = 0, legend = FALSE)
ggsave(plot = p2, filename = "HMMer_pHMM/phylogenetic_annotation.pdf", 
       scale = 2.5, useDingbats = FALSE)

## add a legend
p2 <- gheatmap_mod(p, info, offset = 0.1, width = 0.1, font.size = 1, 
                   colnames_angle = -45, hjust = 0, legend = TRUE)
...
p <- ggplot(tree, mapping=aes(x,y), layout = "circular", ladderize = TRUE, 
    ...
    geom_nodepoint(aes(color = support), size = 0.02, pch = 20) 
p <- ggtree:::layout_circular(p)
p <- p + geom_tiplab(label=tree@phylo$tip.label, aes(angle = angle), size = 0.2) + 
    scale_color_continuous(low = "red", high = "green") 
ggsave(plot=p, scale = 2.5, useDingbats = FALSE, 
    filename = "HMMerphylogenetic_bootstrapsupport.pdf")
